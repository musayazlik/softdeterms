...
const PostCreate = () => {
  ...
  const datalar = useSelector((state: any) => state.posts.data)
  const stateData = datalar?.filter((item: any) => item._id === router.query.id)
  console.log(stateData)

  return (
    <>
      <Head>
        <title>{stateData[0]?.title}</title>
        <meta name='description' content={stateData[0]?.description} />
        <meta name='author' content={stateData[0]?.author} />
      </Head>
      <Layout>
